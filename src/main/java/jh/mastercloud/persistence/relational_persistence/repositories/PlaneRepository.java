...
public interface PlaneRepository extends JpaRepository<Plane, Long> {
  ...
  @Query(value ="SELECT p.plate_number , m.name , m.surname "
			+ "from plane p left join mechanic m "
			+ "on JSON_CONTAINS(JSON_EXTRACT(p.maintenances, '$[*].mechanic_id' ), CAST(m.id as JSON))=1", nativeQuery = true)
  List<PlaneMechanicNameSurnameDto> findPlanesAndMechanicsByJSONFieldNoAsAttributes();
